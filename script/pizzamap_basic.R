...
map <- leaflet(countries) %>% #enchaine les fonctions 
  ...
  addMarkers(data = data,
             ...
             lat = ~lat,
             group = "Pizzerias",
             popup = ~paste("<b>", name, "</b>" ,"<br>", "<br>","This place is up to take away :", take_away,"</b>","<br>","<img src = '",link,"'style='height: 100%; object-fit: contrain'/>",
                            sep = " ")) %>% #langage HTML 
  ...
  addLayersControl(overlayGroups = c("Pizzerias", "Countries"),
                   options = layersControlOptions(collapsed = FALSE)) #pour les choses à côté de la carte 
